...
export default class MessageController {
    ...
    async deleteOneMessage(request, response) {
        ...
        const [message] = await db_connection('user_message').where({ id: Number(messageId) }).select();

        if (!message) return response.status(400).send();

        if (message.sender_identifier === user.id)
            await db_connection('user_message')
                .where({
                    id: Number(messageId)
                }).update({
    ...
    async sendOneMessage(request, response) {
        ...
        const [userExits] = await db_connection('usuarios').where({ id: Number(receiverId) }).select();
